...
import { ProdutoForm, ProdutoView } from "@/components/produtos";
...
export default function ProdutoCard({ produto, onUpdate, onDelete, onToggleActive }) {
  ...
  return (
    <>
      <div className="bg-card border-border/10 hover:border-primary/50 space-y-3 rounded-xl border p-4 transition-all duration-300 hover:shadow-md">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <h3 className="text-lg font-semibold">{produto.nome}</h3>
            <p className="text-muted-foreground text-sm">{produto.marca}</p>
          </div>
        ...
        <div className="space-y-1">
          {!hidePrices && (
            <div className="text-lg font-bold">{formatCurrencyEUR(produto.preco_venda)}</div>
          )}
          {produto.tipo && <p className="text-muted-foreground text-sm">Tipo: {produto.tipo}</p>}
          {produto.peso && <p className="text-muted-foreground text-sm">Peso: {produto.peso}g</p>}
        </div>

        <div className="flex items-center justify-end gap-1 pt-2">
          <Button variant="ghost" size="icon" onClick={handleView} className="h-8 w-8">
            <Eye className="h-4 w-4" />
          </Button>

          <Button variant="ghost" size="icon" onClick={handleAttachments} className="h-8 w-8">
            <IconWithBadge
          ...
          {!isCollaborator && !hidePrices && (
            <AlertDialog>
              ...
              <AlertDialogContent className="bg-card border-none shadow-2xl">
                ...
                <AlertDialogFooter>
                  <AlertDialogCancel className="bg-popover border-border/20">
                    Cancelar
                  </AlertDialogCancel>
      ...
      <Dialog open={showView} onOpenChange={setShowView}>
        <DialogContent className="bg-background max-h-[90vh] w-[95vw] max-w-4xl overflow-y-auto border-none shadow-2xl">
          <DialogHeader>
      ...
      <Dialog open={showAttachments} onOpenChange={setShowAttachments}>
        <DialogContent className="bg-background max-w-4xl border-none shadow-2xl">
          <DialogHeader>
