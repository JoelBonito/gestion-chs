...
export default function StatCard({
  ...
  const getVariantStyles = (v: string) => {
    switch (v) {
      ...
      default:
        return "text-primary border-border bg-card";
    }
  };
  ...
  return (
    <Card
      className={cn(
        "bg-card relative h-full overflow-hidden border border-[var(--border)] transition-all duration-300",
        className
      )}
    >
          ...
          {icon && (
            <div
              className={cn(
                ...
                getIconContainerStyles(variant)
              )}
            >
              {icon}
            </div>
        ...
        <div className="mt-2 flex flex-col gap-1">
          ...
          {(trend || subtitle) && (
            <div className="mt-1 flex items-center gap-2 text-xs">
              ...
              {subtitle && (
                <span className="line-clamp-1 truncate text-[var(--text-tertiary)]">
                  {subtitle}
                </span>
              )}
            </div>
