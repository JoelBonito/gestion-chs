...
export const AttachmentManager: React.FC<AttachmentManagerProps> = ({
  ...
  const t = (k: string) => {
    const d: Record<string, { pt: string; fr: string }> = {
      ...
      "Erro ao carregar anexos": {
        pt: "Erro ao carregar anexos",
        fr: "Erreur lors du chargement des annexes",
      },
      "Anexo adicionado": { pt: "Anexo adicionado", fr: "Annexe ajoutée" },
      "Erro ao anexar arquivo": {
        pt: "Erro ao anexar arquivo",
        fr: "Erreur lors de l’ajout de l’annexe",
      },
    };
    return d[k]?.[lang] || k;
  };
  ...
  return (
    <div
      className={`space-y-4 rounded-xl border p-4 ${useTertiaryLayer ? "bg-popover border-border/40" : "bg-card border-border/50"}`}
    >
