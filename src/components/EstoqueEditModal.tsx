...
export function EstoqueEditModal({
  ...
  const getInputClassName = (valor: number) => {
    if (valor < 0)
      return "border-destructive text-destructive font-semibold bg-destructive/10 dark:bg-destructive/20";
    if (valor < 200)
      return "border-warning text-warning font-semibold bg-warning/10 dark:bg-warning/20";
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setEstoque({ ...estoque, garrafas: parseInt(e.target.value) || 0 })
                }
                className={getInputClassName(estoque.garrafas)}
              />
              {estoque.garrafas < 200 && (
                <p className="text-muted-foreground text-xs">
                  {estoque.garrafas < 0 ? "⚠️ Estoque negativo" : "⚠️ Estoque baixo (< 200)"}
                </p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              {estoque.tampas < 200 && (
                <p className="text-muted-foreground text-xs">
                  {estoque.tampas < 0 ? "⚠️ Estoque negativo" : "⚠️ Estoque baixo (< 200)"}
                </p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              {estoque.rotulos < 200 && (
                <p className="text-muted-foreground text-xs">
                  {estoque.rotulos < 0 ? "⚠️ Estoque negativo" : "⚠️ Estoque baixo (< 200)"}
                </p>
              )}
            </div>
          </div>
