...
export default function Clientes() {
  const { canEdit } = usePermissions(); // ✅ não repete query a cada render
  ...
  const fetchClientes = async () => {
    try {
      ...
      let query = supabase
        .from("clientes")
        .select("id, nome, email, telefone, endereco, active, created_at")
        .order("created_at", { ascending: false });

      // aplica filtro direto no banco
      query = query.eq("active", !showInactive);
            ...
            <Card key={cliente.id}>
              <CardContent className="p-4">
                ...
                <p className="text-sm mt-2">
                  Status: {cliente.active ? "Ativo ✅" : "Inativo ❌"}
                </p>
              </CardContent>
            </Card>
          ))}
