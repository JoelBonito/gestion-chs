...
-- First, let's check current auth.users to get a valid UUID for backfill
-- We'll use the first user ID found, or create a system user if none exists
...
-- For fornecedores table (created_by already exists)
ALTER TABLE public.fornecedores
ADD COLUMN IF NOT EXISTS deactivated_at timestamptz,
ADD COLUMN IF NOT EXISTS deactivated_reason text;

-- For encomendas table
ALTER TABLE public.encomendas
ADD COLUMN IF NOT EXISTS created_by uuid;

-- Backfill created_by columns with first available auth user
-- Get first user ID from auth.users for backfill
DO $$
DECLARE
    first_user_id uuid;
BEGIN
